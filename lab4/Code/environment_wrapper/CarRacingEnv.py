...
class CarRacingEnvironment:
	def __init__(self, N_frame=4, test=False, human=False):
		self.test = test
		if self.test:
			self.env = gym.make('CarRacing-v2', render_mode="human" if human else "rgb_array")
		else:
	...
	def step(self, action):
		...
		if road_pixel_count < 10:
			terminates = True
			reward = -100
		# my own reward function
		reward += 0.1 * road_pixel_count - 0.1 * grass_pixel_count 
	...
	def reset(self, seed=None):
		obs, info = self.env.reset() if seed is not None else self.env.reset(seed=seed)
		self.ep_len = 0
		obs = cv2.cvtColor(obs, cv2.COLOR_BGR2GRAY) # 96x96
