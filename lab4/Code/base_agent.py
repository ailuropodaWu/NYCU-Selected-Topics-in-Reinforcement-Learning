...
class TD3BaseAgent(ABC):
	...
	def evaluate(self, save_video=False):
		...
		for episode in range(self.eval_episode):
			total_reward = 0
			seed = self.seed_list[episode % len(self.seed_list)]
			state, infos = self.test_env.reset(seed=seed)
			if save_video:
				out = cv2.VideoWriter(f'./result/output_{episode}.mp4', fourcc, 20.0, (600,  400)) 
			for t in range(10000):
				screen = self.test_env.render()
				if save_video:
					out.write(screen)
				...
				if terminates or truncates:
					print(
						'Episode: {}\tSeed: {}\tLength: {:3d}\tTotal reward: {:.2f}'
						.format(episode+1, seed, t, total_reward))
					all_rewards.append(total_reward)
					break
			if save_video:
				cv2.imwrite(f'./result/test_{episode}.png', screen)
				out.release()
