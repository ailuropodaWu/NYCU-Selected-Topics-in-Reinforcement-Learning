...
public:
	...
	virtual float update(const board& b, float u) {
		...
		float updated_value = 0.0;
		for (int i = 0; i < iso_last; i++) {
			size_t idx = indexof(isomorphic[i], b);
			operator[](idx) += u_split;
			updated_value += operator[](idx);
		}
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				float location_prob = 1.0 / num;
				for (int i = 0; i < num; i++) {
					for (int pop_num = 0; pop_num < 2; pop_num++) {
						board pop_board = move->after_state();
						pop_board.set(space[i], pop_num + 1);
						float pop_num_prob = pop_num ? 0.1 : 0.9;
						esti_value += location_prob * pop_num_prob * estimate(pop_board);
					}
				}
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for (auto it = path.rbegin(); it != path.rend(); ++it) {
			state& s = *it;
			float td_error = target - estimate(s.before_state());
			target = s.reward() + update(s.before_state(), alpha * td_error); 
		}
	}
