...
class DQNBaseAgent(ABC):
	def __init__(self, config):
		...
		self.replay_buffer = [ReplayMemory(int(config["replay_buffer_capacity"])) for _ in range(self.num_envs)]
	...
	def train(self):
		...
		while self.total_time_step <= self.training_steps:
			...
			episode_len = np.zeros((self.num_envs, ))
			episode_idx += 1
			while True:
				...
				for i in range(self.num_envs):
					self.replay_buffer[i].append(observation[i], [action[i]], [reward[i]], next_observation[i], [int(terminate[i])])
				...
				if terminate.any() or truncate.any():
					self.writer.add_scalar('Train/Episode Reward', episode_reward.mean(), self.total_time_step)
					self.writer.add_scalar('Train/Episode Len', episode_len.mean(), self.total_time_step)
					print(f"[{self.total_time_step}/{self.training_steps}]  episode: {episode_idx}  episode reward: {episode_reward.mean()}  episode len: {episode_len.mean()}  epsilon: {self.epsilon}")
					break
