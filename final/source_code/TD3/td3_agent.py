...
from models.racecar_model import ActorNetSimple, CriticNetSimple
from environment_wrapper.WrappedRaceEnv import WrappedRaceEnv
import random
from base_agent import OUNoiseGenerator, GaussianNoise

class TD3Agent(TD3BaseAgent):
	def __init__(self, config):
		super(TD3Agent, self).__init__(config)
		# initialize environment
		self.env = WrappedRaceEnv(
      		N_frame=4, 
			scenario=config['scenario'], 
			motor_range=config["motor_range"], 
			steer_range=config["steer_range"], 
			test=False
        )
		self.test_env = WrappedRaceEnv(
      		N_frame=4, 
			scenario=config['scenario'], 
			motor_range=config["motor_range"], 
			steer_range=config["steer_range"], 
	...
	def decide_agent_actions(self, state, sigma=0.0):
		...
		with torch.no_grad():
			...
			action = self.actor_net(state).cpu().numpy().squeeze() + sigma * self.noise.generate()
		return action
	...
	def update_behavior_network(self):
		...
		with torch.no_grad():
			...
			a_next = self.target_actor_net(next_state) + torch.clip(torch.randn_like(action), torch.tensor(-0.5).to(self.device), torch.tensor(0.5).to(self.device))
